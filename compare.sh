...
time=`date +%FT%H%M%S`
output="cluster-comparision.txt"
if [ -f "$output" ];then
rm -rf $output
fi
...
compare()
{
 cluster1_name=`echo ${comparefilelist[0]} | awk -F '-' '{print $3}'`
 cluster2_name=`echo ${comparefilelist[$filepos]} | awk -F '-' '{print $3}'`
...
read ssloption
statusdump="/tmp/ambari-servicedump-$clustername-$hostname.txt"
servicefile="/tmp/cluster-service"
...
echo -e "${RED}Unable to download cluster config.Please check logfile under /tmp/cluster-service${NC}"
exit 1
fi
echo -e "\nDownloading component configuration..  Location: $statusdump.. \n"
for i in `cat  $servicefile | grep -i tag -B1 | grep "{" | awk  '{print $1}' | sed -e 's/^"//'  -e 's/"$//'`
do
if echo "$ssloption" | grep -iq "^y" ;then
/var/lib/ambari-server/resources/scripts/configs.sh -u $username -p $passwd -port $port -s get $hostname $clustername $i >>$statusdump
returncode=`echo $?`
else
/var/lib/ambari-server/resources/scripts/configs.sh -u $username -p $passwd get $hostname $clustername $i >>$statusdump
returncode=`echo $?`
fi
if [ $returncode != 0 ];then
...
perl -pe 's/\x1b\[[0-9;]*[mG]//g' $output > cluster-comparison-$time.txt 
echo -e "${GREEN}\nPlease review the file cluster-comparison-$time.txt for cluster config variations${NC}"
